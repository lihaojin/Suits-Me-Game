...
public:
    ...
    std::string get_minsuit()
    {
        ...
        std::string min_suit;
        if(std::min(std::min(H,D), std::min(C,S)) == H) min_suit  = "H";
    ...
    void auto_pass(Player &p)
    {
        ...
        for(int i=0; i<hand.size(); ++i)
        {
            if(hand[i].substr(0,1) == my_min && hand[i].substr(0,1) == p.get_minsuit())
            {
                passCard(p,i);
                break;
            }
            else if(hand[i].substr(0,1) == my_min && hand[i].substr(0,1) != p.get_maxsuit())
            {
                passCard(p,i);
                break;
            }
            else if(hand[i].substr(0,1) == my_min)
            {
                passCard(p,i);
                break;
            }
